...
def run_process(template_path, source_path, output_dir, runner_path):
    ...
    final_name = f"{username}_{ts}H_({Path(source_path).name.replace(".xlsx", "")}).xlsx"
    ...
    try:
        ...
        time.sleep(2) # important for race condition so that python UI does not misinterpret excel's state
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Hybrid Excel Merge Runner v4.3")
